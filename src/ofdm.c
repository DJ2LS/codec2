...
static void allocate_tx_bpf(struct OFDM *ofdm) {
    ...
    else if (!strcmp(ofdm->mode, "datac0") || !strcmp(ofdm->mode, "datac3") || !strcmp(ofdm->mode, "ofdm_adv")) {
...
void ofdm_hilbert_clipper(struct OFDM *ofdm, complex float *tx, size_t n) {
    ...
    if (ofdm->tx_bpf_en) {
        assert(!strcmp(ofdm->mode, "700D") 
               || !strcmp(ofdm->mode, "700E")
               || !strcmp(ofdm->mode, "2020") 
               || !strcmp(ofdm->mode, "2020B")
               || !strcmp(ofdm->mode, "2020C")
               || !strcmp(ofdm->mode, "datac0") 
               || !strcmp(ofdm->mode, "datac3") 
               || !strcmp(ofdm->mode, "ofdm_adv"));
        assert(ofdm->tx_bpf != NULL);
